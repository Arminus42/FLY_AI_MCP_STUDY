...
class MCPClient:
    ...
    async def process_tool_call(self, tool_call) -> ChatCompletionToolMessageParam:
        ...
        # --- [ADDED] PRINT RESULT ---
        # Convert the content to a string for easy reading
        result_str = str(call_tool_result.content)
        print(f"[DEBUG] Tool returned: {result_str[:200]}...") # Print first 200 chars
        # ----------------------------
    ...
    async def workflow_loop(self, prompt:str):
        ...
        system_instruction = (
            "You are an automated software testing assistant. "
            f"Your goal is to generate pytest unit tests.\n\n"
            "CRITICAL OUTPUT RULES:\n"
            f"1. All generated test files must be saved in the directory: '{results_dir}'\n"
            "2. When writing files, ensure the filename follows 'test_<original_name>.py'.\n"
            "3. Do not ask for user confirmation. Just write the files."
        )
